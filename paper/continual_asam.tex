...
\begin{abstract}
Continual learning with Transformer-style attention suffers from a structural tension between plasticity and interference: new tasks require routing flexibility, while previously learned tasks require stable reuse of attention structure. Existing sparse-attention methods mainly target computational efficiency, and existing continual-learning routing methods typically do not expose an explicit sparse-attention geometry that can be diagnosed and controlled across task stages. We present \textbf{Continual ASAM}, a continual-learning extension of adaptive sparse attention in which sparse pattern selection is mediated by learned prototype routing. The method combines four ingredients inside one sparse-attention controller: (1) prototype-routed sparse attention-pattern selection, (2) capacity-constrained Sinkhorn routing that defines prototype usage and deficit statistics, (3) transport-aware prototype lifecycle updates including split, merge, reset, and relocation, and (4) a closed diagnostic loop that exports transport loss, routing drift, and excess statistics for online adaptation. We show that the resulting controller admits a coherent monitoring interpretation: transport loss is an $\epsilon$-consistent estimator of the exact transport cost, routing KL decomposes route drift into an entropy term and a memory-consistency term, and prototype excess is a geometric proxy for routing concentration above the uniform-collision floor. On the task-incremental multi-head Split AG News protocol used for the current reported tables, prototype routing reports lower average forgetting than task-conditioned routing under extended training (3 epochs, BPE tokenization): $0.0000$ vs.\ $+0.0417$, at a small accuracy cost ($0.5104$ vs.\ $0.5312$); both differences are within seed noise at the 64/32-sample scale, so we present the exported routing-geometry traces, not accuracy gains, as the primary contribution. Operator-level ablation confirms that transport-aware relocation is the most important lifecycle component. We provide runnable pipelines, multi-seed ablations, and stage-wise diagnostic reports that export routing-geometry traces for monitoring; on longer task streams where forgetting actually occurs, the traces remain computable and their correlation with forgetting is preliminary and dataset-dependent (Sec.~\ref{sec:longstream}). Our claim is not the invention of sparse attention, Sinkhorn routing, or prototype memory in isolation, but the integration of these mechanisms into a unified continual sparse-routing controller with explicit, exportable routing-geometry variables and an executable evaluation pipeline.
\end{abstract}
...
\begin{enumerate}
    ...
    \item \textbf{Same-variable closed loop.} One set of routing-geometry variables (transport gap, routing KL, excess) serves simultaneously as optimization targets, lifecycle triggers, and exported diagnostics, making the routing design surface measurable end to end.
    \item \textbf{Reproducible evaluation pipeline.} We provide a one-command benchmark suite whose configuration is embedded in every artifact, whose tables are guarded by consistency tests, and which exports stage-wise forgetting together with routing-geometry traces.
\end{enumerate}
...
We provide three results that formalize what the Continual ASAM routing-geometry variables measure. Proofs are in Appendix~\ref{sec:proofs}. The results are deliberately modest: the first two formalize properties of the quantities we monitor, and the final connection to forgetting is presented as an informal interpretation rather than a theorem.
...
\paragraph{Continual Protocol.} The reported results use Split AG News with 2 classes per task, local per-task labels, task-specific classifier heads, and oracle task identifiers at evaluation. This is a task-incremental multi-head protocol rather than a strict single-head class-incremental protocol. Sequence length is capped at 128, batch size is 8, and AdamW uses learning rate $3 \times 10^{-4}$. All reported text tables use 64 training samples and 32 validation samples per task; the BPE strategy table uses 3 random seeds and the byte-level ablations use 2 random seeds. Table~\ref{tab:strategy_ablation_main} and Table~\ref{tab:operator_ablation} use the method's default replay setting (replay batch size 4). For the baseline comparison in Table~\ref{tab:baseline_comparison}, all method rows run \emph{without} replay so that the comparison isolates the continual-learning mechanism itself, and ER/A-GEM are included as explicit replay-based reference rows (replay batch size 4).
...
We also note a null result in Table~\ref{tab:strategy_ablation_main}: the four prototype controller variants (no adaptation, correlation, dual transport, and meta-secant) share identical aggregate accuracy and forgetting ($0.5104$ and $0.0000$). The adaptive controllers do not measurably change outcomes at this scale, so we present them as interchangeable implementations of the same monitoring loop and treat the exported diagnostics (transport gap, routing stability, lifecycle counts), rather than the controller choice, as the framework's contribution.
...
Two observations in Table~\ref{tab:operator_ablation} deserve explicit discussion. First, \texttt{no\_transport} (transport weight $0$) and \texttt{sinkhorn\_topk} produce identical accuracy and forgetting ($0.4844$ and $-0.0938$): at this scale the transport regularizer has no measurable effect on the final metrics, so the transport mechanism should be evaluated through the exported transport-gap traces rather than accuracy deltas. Second, disabling merge or relocation degrades accuracy ($0.5000$ and $0.4688$) relative to the full configuration, with relocation showing the largest effect; this motivates our claim that the lifecycle operators, not the transport term, drive the observable differences at this scale.

\subsection{Comparison with Standard Continual Learning Baselines}

We compare Continual ASAM against plain fine-tuning, standard regularizers (EWC, SI, MAS), and replay-based methods (ER, A-GEM) under a unified byte-level setup (char tokenizer, dim=64, 1 layer, 1 epoch, 64/32 train/validation samples per task, 2 seeds). For fairness, every method uses the same task-incremental multi-head protocol with per-task classifier heads and oracle task identifiers at evaluation, and all method rows (fine-tune, EWC, SI, MAS, and both Continual ASAM variants) run \emph{without} replay so the comparison isolates the continual-learning mechanism itself; ER and A-GEM are explicit replay-based reference rows (replay batch size 4). EWC/SI/MAS use regularizer strengths selected from a small grid ($\{100,1000,5000\}$ for EWC; $\{0.1,1,10\}$ for SI/MAS) by validation accuracy; the grid is flat at this scale, so the selection does not affect the reported numbers. Table~\ref{tab:baseline_comparison} reports mean accuracy, forgetting, and backward transfer. All methods achieve accuracy in $0.4688$--$0.5156$ and forgetting in $0.0000$--$0.0312$; bootstrap 95\% intervals for the key differences touch zero (Appendix~\ref{sec:bootstrap}). Continual ASAM (prototype) achieves the highest average accuracy ($0.5156$) with forgetting $0.0312$ that matches the task-conditioned row and slightly exceeds the zero-forgetting rows of the simpler methods; ER and A-GEM reach $0.5000$ accuracy with zero forgetting. The prototype-routed variant additionally exports per-stage transport gaps, routing stability, and prototype lifecycle statistics that none of the standard baselines can provide.

\begin{table}[t]
\centering
\caption{Comparison against standard continual learning baselines under a unified task-incremental multi-head protocol (char tokenizer, dim=64, 1 layer, 1 epoch, 2 seeds, 64/32 samples per task). Method rows run without replay; ER/A-GEM are replay-based references (batch size 4). EWC/SI/MAS report the selected regularizer strength. Lower forgetting is better.}
\label{tab:baseline_comparison}
\begin{tabular}{lcccc}
\toprule
\textbf{Method} & \textbf{Accuracy} & \textbf{Forgetting} $\downarrow$ & \textbf{BWT} $\uparrow$ & $\lambda$ \\
\midrule
fine\_tune (no CL) & $0.4844 \pm 0.0156$ & $0.0000 \pm 0.0000$ & $0.0000 \pm 0.0000$ & --- \\
EWC & $0.5000 \pm 0.0000$ & $0.0000 \pm 0.0000$ & $0.0000 \pm 0.0000$ & $100$ \\
SI & $0.4844 \pm 0.0156$ & $0.0000 \pm 0.0000$ & $0.0000 \pm 0.0000$ & $0.1$ \\
MAS & $0.5000 \pm 0.0000$ & $0.0000 \pm 0.0000$ & $0.0000 \pm 0.0000$ & $0.1$ \\
ER & $0.5000 \pm 0.0000$ & $0.0000 \pm 0.0000$ & $0.0000 \pm 0.0000$ & --- \\
A-GEM & $0.5000 \pm 0.0000$ & $0.0000 \pm 0.0000$ & $0.0000 \pm 0.0000$ & --- \\
Continual ASAM (task) & $0.4688 \pm 0.0000$ & $0.0312 \pm 0.0312$ & $-0.0312 \pm 0.0312$ & --- \\
Continual ASAM (prototype) & $\mathbf{0.5156} \pm \mathbf{0.0156}$ & $0.0312 \pm 0.0312$ & $-0.0312 \pm 0.0312$ & --- \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Long Task Streams and the Forgetting Regime}
\label{sec:longstream}

The two-task protocol does not exercise forgetting. To test whether the exported diagnostics are informative in a regime where forgetting actually occurs, we run longer task streams in a class-incremental single-head protocol (global labels, one shared classifier head, no oracle task identifiers at evaluation) on Split DBPedia-14 (7 tasks) and CIFAR-10 rendered as 256 pixel tokens (5 tasks), with $D=128$, 2 layers, 3 epochs, 256/128 train/validation samples per task, and 3 seeds.

Table~\ref{tab:longstream} reports the results. All methods suffer severe catastrophic forgetting: final accuracy collapses to chance or to the last task's classes (fine-tune, EWC, and ER end at $0.1429$ on DBPedia-14, i.e., only the two most recent classes remain correct), and average forgetting reaches $0.36$--$0.55$. Prototype routing reports the lowest forgetting ($0.3639$ on DBPedia-14; $0.5192$ on CIFAR-10) but its final accuracy is at or below chance ($0.0655$ and $0.1111$), whereas ER---the replay reference---reaches $0.1429$ with forgetting $0.5083$ on DBPedia-14. Disabling the transport regularizer removes its negative effect on final accuracy on DBPedia-14 ($0.1345$ vs.\ $0.0655$) and is neutral on CIFAR-10, so the transport term has no positive effect at this scale.

Stage-wise correlations between the exported diagnostics and forgetting are mixed: on CIFAR-10 the transport gap tracks stage forgetting (mean Pearson $r = 0.79$ over 3 seeds), while on DBPedia-14 the correlation is near zero ($r = -0.14$); routing stability shows a weak positive correlation on DBPedia-14 ($r = 0.38$). With three seeds per setting these estimates are not conclusive, but they demonstrate that the diagnostics remain computable and exportable in a regime where forgetting actually occurs, and that their relationship to forgetting is dataset-dependent.

We report these results because they delimit what the framework currently offers: a measurable, exportable routing geometry in both easy and hard regimes, without evidence that prototype routing prevents catastrophic forgetting in long class-incremental streams.

\begin{table}[t]
\centering
\caption{Class-incremental single-head long task streams (DBpedia-14: $T=7$; CIFAR-10: $T=5$; $D=128$, 2 layers, 3 epochs, 256/128 samples per task, 2--3 seeds). Final accuracy and average forgetting; lower forgetting is better.}
\label{tab:longstream}
\begin{tabular}{lccc}
\toprule
\textbf{Dataset} & \textbf{Method} & \textbf{Accuracy} & \textbf{Forgetting} \\
\midrule
DBpedia-14 & fine\_tune & $0.1429 \pm 0.0000$ & $0.5542 \pm 0.0125$ \\
DBpedia-14 & EWC & $0.1429 \pm 0.0000$ & $0.5167 \pm 0.0083$ \\
DBpedia-14 & ER (replay) & $0.1429 \pm 0.0000$ & $0.5083 \pm 0.0417$ \\
DBpedia-14 & ASAM prototype & $0.0655 \pm 0.0269$ & $0.3639 \pm 0.0104$ \\
DBpedia-14 & ASAM no-transport & $0.1345 \pm 0.0118$ & $0.4111 \pm 0.0515$ \\
CIFAR-10 & ASAM prototype & $0.1111 \pm 0.0157$ & $0.5192 \pm 0.0157$ \\
CIFAR-10 & ASAM no-transport & $0.1083 \pm 0.0068$ & $0.5192 \pm 0.0157$ \\
\bottomrule
\end{tabular}
\end{table}
...
All experiments are reproducible through the released codebase. The one-command script \texttt{scripts/run\_continual\_paper\_suite.py} executes the benchmark, runs the multi-seed ablation, and writes a complete suite manifest plus paper-ready summary report. The canonical artifacts for the tables are \texttt{experiments/paper\_suite/r2\_agnews\_bpe\_3ep.json} (Table~\ref{tab:strategy_ablation_main}), \texttt{experiments/paper\_suite/continual\_operator\_ablation.json} (Table~\ref{tab:operator_ablation}), \texttt{experiments/paper\_suite/r3\_baseline\_comparison.json} (Table~\ref{tab:baseline_comparison}), \texttt{experiments/paper\_suite/continual\_benchmark.json} (single-run diagnostics), \texttt{experiments/paper\_suite/bootstrap\_ci.json} (bootstrap intervals, Appendix~\ref{sec:bootstrap}), and the long-stream artifacts \texttt{r4\_dbpedia\_longstream.json}, \texttt{r4\_cifar10\_longstream.json}, \texttt{r4\_dbpedia\_baselines.json}, \texttt{r4\_analysis.json} (Table~\ref{tab:longstream}). Every number in the paper tables is traceable to a saved JSON, CSV, or Markdown artifact, and the table-to-artifact mapping is enforced by a consistency test in the repository.
...
\textbf{Benchmark regime.} The two-task task-incremental multi-head protocol with per-task heads and oracle task identifiers produces near-zero forgetting for every method, so the two-task protocol does not exercise forgetting. On longer class-incremental single-head streams (Sec.~\ref{sec:longstream}), forgetting does occur, but every method suffers severe catastrophic forgetting: final accuracy collapses to chance or to the last task's classes, prototype routing's lower forgetting comes with at-or-below-chance final accuracy, and the transport regularizer has no positive effect. The stage-wise diagnostics remain computable, but their correlation with forgetting is dataset-dependent and based on only three seeds, so the evidence that they track forgetting is preliminary.
...
We presented Continual ASAM, a continual-learning extension of adaptive sparse attention that combines prototype-routed sparse support selection, capacity-constrained optimal transport, transport-aware lifecycle control, and a closed diagnostic loop into one unified controller. The method is not a claim to invent each component in isolation, but rather an argument that continual sparse-routing control becomes more observable when the same routing-geometry variables appear in the optimization surrogate, the lifecycle rules, the diagnostic exports, and the online adaptation loop. Our reproducible benchmark suite demonstrates that these variables are computable, exportable, and guarded by consistency tests, and we report the scale at which their differences are currently measurable (or not): at the 64/32-sample, two-task scale, accuracy and forgetting differences sit within seed noise, and correlating the diagnostics with forgetting requires task streams long enough for forgetting to occur. We believe the design principle---route, measure, diagnose, and adapt from the same variables---may generalize beyond the current text classification setting to broader modular continual-learning systems, and we treat the honest characterization of when the diagnostics are informative as part of the contribution.
...
For the key comparisons, we resample the per-seed values with replacement (2{,}000 draws, fixed seed) and report percentile 95\% intervals. In Table~\ref{tab:strategy_ablation_main}, prototype minus task-conditioned forgetting is $-0.0418$ with interval $[-0.1042, +0.0208]$, and the accuracy difference is $-0.0208$ with interval $[-0.0521, +0.0104]$. In Table~\ref{tab:baseline_comparison}, prototype minus fine-tuning accuracy is $+0.0313$ with interval $[0.0000, +0.0625]$, prototype minus ER accuracy is $+0.0156$ with interval $[0.0000, +0.0312]$, and the corresponding forgetting differences are $+0.0312$ with intervals touching zero. All intervals include or touch zero, consistent with the noise-level characterization in the main text. The full results are in \texttt{experiments/paper\_suite/bootstrap\_ci.json}.
...
\begin{itemize}
    ...
    \item \textbf{Artifacts.} All JSON summaries, CSV tables, and PNG plots are generated by the paper suite scripts; the canonical tables map to \texttt{r2\_agnews\_bpe\_3ep.json}, \texttt{continual\_operator\_ablation.json}, \texttt{r3\_baseline\_comparison.json}, \texttt{continual\_benchmark.json}, and \texttt{bootstrap\_ci.json}, enforced by \texttt{tests/test\_paper\_artifacts\_consistency.py}.
