...
\textbf{Benchmark regime.} The two-task task-incremental multi-head protocol with per-task heads and oracle task identifiers produces near-zero forgetting for every method, so the current experiments do not exercise a regime in which forgetting actually occurs; the stricter class-incremental single-head smoke check operates near chance. Whether the exported diagnostics track forgetting in a regime where forgetting happens therefore remains open, and answering it requires longer task streams (e.g., $T \geq 5$) or stricter protocols.
