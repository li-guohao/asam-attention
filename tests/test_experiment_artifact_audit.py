...
def test_semantic_duplicates_ignore_output_paths(tmp_path):
    ...
    assert sorted(grouped["path"] for grouped in summary["semantic_duplicate_groups"][0]["files"]) == [
...
def test_schema_provenance_ratings(tmp_path):
    ...
    _write_json(current / "continual_ablation.json", {"strategies": [{"strategy": "dual_transport"}]})
    _write_json(
    ...
    _write_json(outdated / "paper_suite_manifest.json", {"config": {"candidate_profile": "default"}})
