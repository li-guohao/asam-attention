"""
Ablation runner for continual ASAM text benchmarks.
"""
...
ABLATION_STRATEGIES = [
    ...
    },
    {
        ...
        "routing_mode": "prototype",
        "adaptive_hyperparameters": True,
        "adaptation_strategy": "dual_transport",
    },
    {
        "name": "meta_secant",
        "routing_mode": "prototype",
...
def build_benchmark_args(args: AblationArgs, strategy: Dict[str, object], seed: int, output_json: Optional[str]) -> RealBenchmarkArgs:
    return RealBenchmarkArgs(
        ...
        dual_transport_trend_weight=float(strategy.get("dual_transport_trend_weight", args.dual_transport_trend_weight)),
        dual_transport_gap_weight=float(strategy.get("dual_transport_gap_weight", args.dual_transport_gap_weight)),
        dual_transport_loss_weight=float(strategy.get("dual_transport_loss_weight", args.dual_transport_loss_weight)),
        dual_topk_min_stages=int(strategy.get("dual_topk_min_stages", args.dual_topk_min_stages)),
        dual_topk_threshold=float(strategy.get("dual_topk_threshold", args.dual_topk_threshold)),
        ...
        prototype_birkhoff_transport_strength=float(
            strategy.get("prototype_birkhoff_transport_strength", args.prototype_birkhoff_transport_strength)
        ),
        prototype_birkhoff_adaptive_gate=bool(
            strategy.get("prototype_birkhoff_adaptive_gate", args.prototype_birkhoff_adaptive_gate)
        ),
        prototype_birkhoff_gap_target=float(
            strategy.get("prototype_birkhoff_gap_target", args.prototype_birkhoff_gap_target)
        ),
        prototype_birkhoff_max_applied_offdiag_mass=float(
            strategy.get(
                "prototype_birkhoff_max_applied_offdiag_mass",
                args.prototype_birkhoff_max_applied_offdiag_mass,
            )
        ),
        prototype_birkhoff_gap_tolerance=float(
            strategy.get("prototype_birkhoff_gap_tolerance", args.prototype_birkhoff_gap_tolerance)
        ),
        prototype_birkhoff_min_effective_strength=float(
            strategy.get(
                "prototype_birkhoff_min_effective_strength",
                args.prototype_birkhoff_min_effective_strength,
            )
        ),
        prototype_prior_strength=float(strategy.get("prototype_prior_strength", args.prototype_prior_strength)),
        prototype_capacity_blend=float(strategy.get("prototype_capacity_blend", args.prototype_capacity_blend)),
        prototype_relocation_strength=float(strategy.get("prototype_relocation_strength", args.prototype_relocation_strength)),
        adaptive_hyperparameters=bool(strategy["adaptive_hyperparameters"]),
        adaptation_strategy=str(strategy["adaptation_strategy"]),
        device=args.device,
        seed=seed,
...
def summarize_result(name: str, result: Dict[str, object]) -> Dict[str, object]:
    theory = result.get("theory_diagnostics", {})
    final_hparams = result.get("final_hyperparameters", {})
    lifecycle = result.get("prototype_lifecycle", [])
    hyper_hist = result.get("hyperparameter_history", [])
    ...
    return {
        "strategy": name,
        "routing_mode": str(result.get("config", {}).get("routing_mode", "task")),
        ...
        "final_stage_forgetting": float((theory.get("stage_forgetting", [0.0]) or [0.0])[-1]),
        ...
        "adaptation_steps": len(hyper_hist),
        "total_resets": total_resets,
        "total_splits": total_splits,
        "final_hyperparameters": final_hparams,
    }


def aggregate_strategy_runs(strategy: str, routing_mode: str, rows: List[Dict[str, object]]) -> Dict[str, object]:
    ...
    final_forgetting_mean, final_forgetting_std = mean_std("final_stage_forgetting")
    ...
    return {
        ...
        "final_stage_forgetting_mean": final_forgetting_mean,
        "final_stage_forgetting_std": final_forgetting_std,
...
def build_markdown_table(rows: List[Dict[str, object]]) -> str:
    ...
    for row in rows:
        lines.append(
            "| {strategy} | {routing_mode} | {avg_accuracy_mean:.4f}?{avg_accuracy_std:.4f} | {avg_forgetting_mean:.4f}?{avg_forgetting_std:.4f} | {backward_transfer_mean:.4f}?{backward_transfer_std:.4f} | {final_transport_gap_mean:.4f}?{final_transport_gap_std:.4f} | {num_runs} |".format(**row)
        )
    return "\n".join(lines)
...
def save_csv(rows: List[Dict[str, object]], path: Path):
    fields = [
        ...
        "final_stage_forgetting_mean",
        "final_stage_forgetting_std",
...
def build_report(summary: Dict[str, object]) -> str:
    ...
    lines = [
        ...
        f"- Number of strategies: `{len(rows)}`",
        f"- Seeds per strategy: `{summary['config']['num_seeds']}`",
...
def run_ablation(args: AblationArgs) -> Dict[str, object]:
    ...
    for strategy in ABLATION_STRATEGIES:
        ...
        for seed_offset in range(args.num_seeds):
            ...
            if output_base is not None:
                ...
                strategy_output = str(
                    output_base.with_name(
                        f"{output_base.stem}_{strategy['name']}_seed{seed}.json"
                    )
                )
            ...
            summarized = summarize_result(strategy["name"], result)
            summarized["seed"] = seed
            strategy_runs.append(summarized)
            per_run_results.append(summarized)

    aggregated = []
    for strategy in ABLATION_STRATEGIES:
        rows = [row for row in per_run_results if row["strategy"] == strategy["name"]]
        aggregated.append(aggregate_strategy_runs(strategy["name"], strategy["routing_mode"], rows))
...
def parse_args() -> AblationArgs:
    parser = argparse.ArgumentParser(description="Run continual ASAM ablations on the text benchmark")
    for field_name, field_def in AblationArgs.__dataclass_fields__.items():
    ...
    return AblationArgs(**vars(namespace))
