...
from ._common import gather_values_by_positions, normalize_attention_mask
from .adaptive_gate import AdaptiveGate
from .sparse_patterns import HierarchicalSparsePattern, LocalSparsePattern, SparsePattern
...
class OptimizedASAMLayer(nn.Module):
    ...
    def _compute_local_attention(
        ...
        if mask is not None:
            window_positions, valid_window_mask = self._get_local_window_indices(
                seq_len, window_size, q.device
            )
    ...
    def _get_strided_attention_indices(
        ...
        local_positions, local_valid_mask = self._get_local_window_indices(
            seq_len, local_window * 2, device
        )
        strided_positions = torch.arange(0, seq_len, stride, device=device)
        query_positions = torch.arange(seq_len, device=device)
        strided_duplicates = (
            query_positions.unsqueeze(-1) - strided_positions.view(1, -1)
        ).abs() <= local_window
    ...
    def forward(
        ...
        if self.pattern_type == "local":
            attn_out = self._compute_local_attention(
                q, k, v, self.window_size, mask=normalized_mask
            )
