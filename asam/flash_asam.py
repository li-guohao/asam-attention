    ...
    def flash_attention_forward(
        ...
        if self.use_flash:
            ...
            return self.flash_attn_func(
                q, k, v, causal=causal, softmax_scale=None  # Use default 1/sqrt(d)
            )
        else:
...
class SparseFlashAttention(nn.Module):
    ...
    def forward(
        self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, pattern_mask: torch.Tensor
    ) -> torch.Tensor:
        """
...
class HybridASAM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        q, k, v = map(
            lambda t: t.reshape(batch, seq_len, self.config.num_heads, self.config.dim_head), qkv
        )
...
def benchmark_flash_vs_standard(seq_lengths=[512, 1024, 2048, 4096], device="cuda"):
    ...
    for seq_len in seq_lengths:
        ...
        try:
            ...
            print(
                f"Seq Len {seq_len}: Standard={time_std:.2f}ms, Flash={time_flash:.2f}ms, Speedup={speedup:.2f}x"
            )

            results.append(
                {"seq_len": seq_len, "standard": time_std, "flash": time_flash, "speedup": speedup}
            )
        except ImportError:
