...
class AdaptiveGate(nn.Module):
    ...
    def __init__(
        ...
        self.pools = nn.ModuleList(
            [nn.AdaptiveAvgPool1d(max(1, dim // (2**i))) for i in range(num_pools)]
        )
    ...
    def extract_features(self, x: torch.Tensor) -> torch.Tensor:
        ...
        attn_weights = F.softmax(
            torch.bmm(multi_scale, x_t).mean(dim=-1, keepdim=True), dim=1
        )  # [batch, pool_dim, 1]
...
class DynamicSparseDenseAttention(nn.Module):
    ...
    def forward(
        ...
        q, k, v = map(
            lambda t: t.reshape(batch, seq_len, self.num_heads, self.dim_head).transpose(1, 2), qkv
        )
